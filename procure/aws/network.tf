...
resource "aws_security_group" "security_group" {
  ...
  ingress {
    ...
    protocol    = "tcp"
    # Allow traffic from configured IPs and load balancer
    cidr_blocks = concat(var.ip_allow_http, ["${aws_eip.lb_eip.public_ip}/32"])
  }
...
resource "aws_eip" "lb_eip" {
  tags = {
    Name = "${var.instance_name}-lb-eip"
  }
}

# Use association to break eip -> security group -> load balancer cycle
